...
export function scrollStatusIntoView() {
  const target =
    getDom().feedback && !getDom().feedback.hidden ? getDom().feedback : getDom().roundTransition;
...
export function beginRoundTransition(duration, label, nextRoundFn) {
  ...
  if (
    !getDom().roundTransition ||
    !getDom().roundTransitionFill ||
    !getDom().roundTransitionLabel
  ) {
