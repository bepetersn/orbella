...
export function setHints(hints = []) {
  ...
  const hintText = hints.map(formatHintText).filter(Boolean).join(getConfig().COPY.hints.separator);
...
export function buildWikipediaUrl(countryName) {
  const normalizedTitle = String(countryName ?? '')
    .trim()
    .replace(/\s+/g, '_');
  return normalizedTitle
    ? `https://en.wikipedia.org/wiki/${encodeURIComponent(normalizedTitle)}`
    : '';
}
...
export function updateStats() {
  log.debug('[round UI] updateStats', {
    numCorrect: getState().store?.numCorrect,
    numPlayed: getState().store?.numPlayed,
    numHintsUsed: getState().store?.numHintsUsed,
  });
...
export function fillNextGuessPill(countryOrName, resultType = 'guess', proximityInfo = null) {
  ...
  const nextPill = Array.from(getDom().guessList.children).find((pill) =>
    pill.classList.contains('empty')
  );
  ...
  nextPill.setAttribute(
    'aria-label',
    `${resultType === 'correct' ? 'Correct guess' : 'Guess'}: ${displayName}`
  );
